...
class AdvancedSearchController extends Controller
{
    ...
    public function show(Request $request, CarStock $car)
    {
        ...
        return view("cars.show", compact('car', 'cover', 'images', 'relatedCars'));
    }

    public function relatedCars($car_id)
    {
        $carStock = CarStock::find($car_id);

        $brand_id = $carStock->car->series->brand_id;

        $nearestCars = CarStock::select('car_id')
            ->whereHas('car.series', function ($query) use ($brand_id) {
    ...
    public function carPartShow(Request $request, CarParts $part)
    {
        ...
        return view("cars.show-part", compact("part", "relatedParts", "cartCount", 'images', 'cover'));
    }
