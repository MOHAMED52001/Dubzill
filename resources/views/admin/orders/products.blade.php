...
<div class="card">
  <h2 class="card-header text-center">{{$order->tracking_number}}</h2>
  <div class="card-body">
    <div class="search mt-2 mb-5">
      ...
      <div class="input-group input-group-merge">
        ...
        <input type="text" class="form-control" placeholder="Search..." aria-label="Search..." aria-describedby="basic-addon-search31" dir="ltr" id="search">
      </div>
    </div>
            ...
            <td>{{ $product->id }}</td>
              ...
              <td>
                ...
                @foreach ($images as $image)
                  <img style="max-width: 50px" src="{{ asset("storage/".$image)}}" alt="">
                @endforeach
              </td>
              <td class="text-center">
                <div class="btn-group">
                  <!-- <button type="button" onclick="showConfirmationModal({{$product->id}}, {{$order->id}})" class="btn btn-outline-danger btn-sm">Delete</button> -->
                  <a href="{{ url('admin/order/product', [$product->id, 'order', $order->id, 'delete']) }}" type="button" class="btn btn-outline-danger btn-sm">delete</a>
...
<div class="modal fade" id="confirmationModal" tabindex="-1" role="dialog" aria-labelledby="confirmationModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    ...
    function showConfirmationModal(ProductId, OrderId) {
      ...
      function deleteProduct(ProductId, OrderId) {
        $.ajax({
          url: '/admin/order/product/' + ProductId + '/order/' + OrderId + '/delete',
          type: 'DELETE',
          data: {
            _token: '{{ csrf_token() }}',
          },
